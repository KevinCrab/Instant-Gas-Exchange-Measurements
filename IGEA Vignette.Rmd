...
The aim of this package is to provide a pipeline of treating the standard output table from LI-6400XT. Measurements are first reformated, merged and screened to see abnormal measurements in ``read.GX`` and ``CS.GXcurve`` function. Then curves can be plotted so that outliers can be spotted and found, using ``plotGX`` and ``Find.GXcurve``. Values will be extracted from curves with function ``GetValue.GXcurve``. At last ``PGMean.GXvalue`` is unsed to calculate plot means and genotype means. ``summary.GXcurve``, ``summary.GXvalue``, ``summary.GXmean`` provide detailed information for corresponding class of data.
