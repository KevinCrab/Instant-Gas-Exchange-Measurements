...
Find.GXcurve = function(table, id = NULL,DuplicateName = FALSE, TooSmall = FALSE, MissRecording = FALSE){
  id_all = vector()
  ...
  names(id_all) = c(rep("input",length(id)),rep("Duplicate name",length(duplicatelist)),rep("too few observations",length(toosmall_names)),rep("Missrecording",length( missrecording)))
  if(length(id_all)==0){
